...
# ── Paths ──────────────────────────────────────────────────────────────────────
# pure_only/ variant: 9 pure-cluster candidates, no blends.
BASE_DIR         = Path(__file__).parent.parent.parent
BALLOTS_PATH     = BASE_DIR / "data" / "outputs" / "pure_only" / "presidential_ballots.csv"
...
def main():
    ...
    active, r_rows, t_rows = winnow(
        ...
        active, survivors_target=8, label="After_Retail_Six"
    )
    all_results  += r_rows
    ...
    active, r_rows, t_rows = winnow(
        ballots_arr[cum_mask], weights[cum_mask],
        active, survivors_target=7, label="After_Pod_A"
    )
    all_results  += r_rows
    all_transfers += t_rows
    ...
    active, r_rows, t_rows = winnow(
        ballots_arr[cum_mask], weights[cum_mask],
        active, survivors_target=6, label="After_Pod_C"
    )
    all_results  += r_rows
    all_transfers += t_rows
